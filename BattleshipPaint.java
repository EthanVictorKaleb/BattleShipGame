...
public class BattleshipPaint extends java.applet.Applet implements ActionListener
{
	//generated by quick fix to get rid of a warning
	...
	private Font fnt72B = new Font("Arial", Font.BOLD, 72);
	...
	int pin_numw = 0;
	int pin_numr = 0;
	Point[] rightredpins = new Point[100];
	Point[] rightwhitepins = new Point[100];
	int rpin_numw = 0;
	int rpin_numr = 0;
	...
	public void paint (Graphics g)
	{
		...
		//draw all the pins based on the board arrays
		//this nested for is for the left board
		for(int i = 0; i <= 9; i++)
		{
			for(int j = 0; j <= 9; j++)
			{
				int val = Game.get_board(i, j);
				int xcoord = 110 + (55*i);
				int ycoord = 46 + (50*j);
				if(val == 2)
				{
					g.drawImage(wp, xcoord, ycoord, 100, 100, this);
				}//end if whitepin
				if(val == 3)
				{
					g.drawImage(rp, xcoord, ycoord, 100, 100, this);
				}//end redpin
			}//end board[y] for
		}//end board[x] for
		
		//this nested for is for the right board
		for(int i = 0; i <= 9; i++)
		{
			for(int j = 0; j <= 9; j++)
			{
				int val = Game.get_otherboard(i, j);
				int xcoord = 730 + (55*i);
				int ycoord = 44 + (50*j);
				if(val == 2)
				{
		...
		//announce if you lose
		if(haveLost)
		{
			g.setFont(fnt72B);
			g.drawString("YOU LOSE", 520, 350);
		}
	...
	public void actionPerformed(ActionEvent e) 
	{
		...
		if(e.getSource() == Connect)
			{
				...
				if(isPlayerOne)
				{
					...
					Game.set_otherboard(9, 9, 9);//DIAGNOSTIC
					repaint();
				}
		...
		if(e.getSource() == b_attack)
			{
				if(!(Game.translate(tf_cl.getText(), tf_cn.getText()).equals("TRY AGAIN")))
				{
					//send the coordinates to the other player 
					OSocklocal.writeSock(Game.translate(tf_cl.getText(), tf_cn.getText()));
					String attack = Game.translate(tf_cl.getText(), tf_cn.getText());
					String[] attacksplit = attack.split(" ");
					int xattack = Integer.parseInt(attacksplit[0]);
					int yattack = Integer.parseInt(attacksplit[1]);
					
					//wait on response from other player
					String hitormiss;
					while((hitormiss = OSocklocal.readSock()) == null)
					{
					...
					{
						Game.set_otherboard(xattack, yattack, 3);
					}
					else if(hitormiss.equals("MISS"))
					{
						Game.set_otherboard(xattack, yattack, 2);
					}
	...
	public void otherTurn()
	{
		...
		String[] move = attackdata.split("\\s+");
		...
		Game.hit(xhit, yhit);
		//tell the other player if it was a hit
		if(Game.get_board(xhit, yhit) == 3)
		{
			OSocklocal.writeSock("HIT");
		}
		else if(Game.get_board(xhit, yhit) == 2)
		{
			OSocklocal.writeSock("MISS");
		}
		
		b_attack.setVisible(true);
		
		haveLost = Game.isEliminated();
